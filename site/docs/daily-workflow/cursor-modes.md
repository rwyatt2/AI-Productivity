...
Think of it like: sometimes you need to talk things through (Chat/Plan mode), sometimes you need to change lots of stuff at once (multi-file/Agent mode), and sometimes you just need to fix one little thing (small edit).
...
2. Switch to **Plan mode** (Shift+Tab) so the AI focuses on planning, not code.
3. Paste Session Kickoff and your Context Pack so the AI knows the project.
4. Ask your question or describe what you're trying to figure out. When you're ready for code, switch to **Agent mode** (Shift+Tab) and move to multi-file mode for the actual edits.
...
2. Make sure you are in **Agent mode** (Shift+Tab to check the mode indicator).
3. Paste your Context Pack (and Session Kickoff + Router if you're starting fresh).
4. Say what you want: e.g. "Add a login button to the header and wire it to the auth service."
5. Review the changes it suggests before you accept them. If something is wrong, say so in a short message.
...
* **You want to think, plan, or get a spec** → Use **Chat in Plan mode**.
* **You want to change lots of files** (feature or refactor) → Use **multi-file (Composer/Agent mode)**.
