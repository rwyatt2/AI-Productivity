...
description: >-
  Initialize a new AI session with the kit protocol defaults. Use when the user
  invokes /session-kickoff, starts a new thread, or says "kickoff" / "start
  session". Sets SPEC-first, 85% gate, one-question protocol, and switch
  commands.
---

# Session Kickoff

Paste or invoke this at the start of a new session so the AI follows the kit
protocol.
...
If `docs/ai/ai-config.md` exists, treat it as authoritative. Do not invent
endpoints, file paths, or dependencies.

## Next step

After kickoff, paste your **Context Pack** (platform type, exposure, data
sensitivity, and 3–7 context items).
